...
double MatrixMath::ComputeDeterminant2(const Matrix &matrix) {
    if (matrix.dimensions().row_count != 2
        && matrix.dimensions().column_count != 2)
...
Matrix MatrixMath::ContractMatrix(const Matrix &matrix,
                                  unsigned int row,
                                  unsigned int column) {
    unsigned int row_count = matrix.dimensions().row_count;
    unsigned int column_count = matrix.dimensions().column_count;
    ...
    if (row_count == 1 || column_count == 1)
        throw std::invalid_argument(
            "ContractMatrix: Matrix too small to contract");
