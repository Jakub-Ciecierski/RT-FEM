...
TEST_F(AABBAlgorithmsTest, FindMin_ProperXMin){
    ...
    EXPECT_EQ(expected_min, rtfem::FindMin(triangle_mesh_->triangles[0], rtfem::AABBCoordinate::X));
}

TEST_F(AABBAlgorithmsTest, FindMin_ProperYMin){
    constexpr float expected_min = 1.0f;
    EXPECT_EQ(expected_min, rtfem::FindMin(triangle_mesh_->triangles[0], rtfem::AABBCoordinate::Y));
}

TEST_F(AABBAlgorithmsTest, FindMin_ProperZMin){
    constexpr float expected_min = 0.0f;
    EXPECT_EQ(expected_min, rtfem::FindMin(triangle_mesh_->triangles[0], rtfem::AABBCoordinate::Z));
}

TEST_F(AABBAlgorithmsTest, FindMax_ProperXMax){
    constexpr float expected_max = 5.0f;
    EXPECT_EQ(expected_max, rtfem::FindMax(triangle_mesh_->triangles[0], rtfem::AABBCoordinate::X));
}

TEST_F(AABBAlgorithmsTest, FindMax_ProperYMax){
    constexpr float expected_max = 9.0f;
    EXPECT_EQ(expected_max, rtfem::FindMax(triangle_mesh_->triangles[0], rtfem::AABBCoordinate::Y));
}

TEST_F(AABBAlgorithmsTest, FindMax_ProperZMax){
    constexpr float expected_max = 3.0f;
    EXPECT_EQ(expected_max, rtfem::FindMax(triangle_mesh_->triangles[0], rtfem::AABBCoordinate::Z));
}
